...
def analyze_run(run_path: str, save_plots: bool = True):
    ...
    summary_table.add_row("Best Perplexity", f"{history['eval/perplexity'].min():.2f}" if 'eval/perplexity' in history else "N/A")
    ...
    if save_plots:
        ...
        console.print(f"[green]✓[/green] Saved training_analysis.png")
        ...
        if any('datadecide' in col for col in history.columns):
            ...
            console.print(f"[green]✓[/green] Saved datadecide_metrics.png")
    ...
    console.print(Panel.fit(
        ...
        f"Files saved to: {output_dir if save_plots else '.'}/",
        border_style="green"
    ))
